...
UserSchema.statics.addImage=(id,file,callback)=>{

  User.findByIdAndUpdate(id, {$set:{filename:file.filename}}, {new: true} , function(err, doc){
...
UserSchema.statics.removeImage=(id,callback)=>{
  User.findByIdAndUpdate(id, {$set:{filename:''}}, {new: true} , function(err, doc){
    if(err) console.log(err);
...
// UserSchema.statics.updatetoken=(id,verifytoken,callback)=>{
//   User.findOneAndUpdate({id: _id}, {$set:{ token:verifytoken}}, {new: true} , function(err, doc){
//     if(err){
